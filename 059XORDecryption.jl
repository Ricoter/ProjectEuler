...
# Julia's XOR: \xor gives ⊻

data = readline("input/059Cipher.txt")
...
ascisum = 0
words = []
for shift=0:2, c=Int('a'):Int('z')
    decrypted = [data[i] ⊻ c for i=1+shift:3:length(data)]
    if minimum(decrypted) == Int(' ') && maximum(decrypted) <= Int('z')
        push!(password, Char(c))
        ascisum += sum(decrypted)
        push!(words, Char.(decrypted))
    end
end
println("Password: $(string(password...))")
println("ASCI sum: $ascisum")

text = join(reshape([words[i][j] for i=1:3, j=1:length(words[1])], 1, :), "")
println("\nText:\n$text")
